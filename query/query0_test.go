...
func TestDateTimeQuery(t *testing.T) {
	...
	// TODO: add test case using since
	// TODO: add test case using aggregation with datetime
	// https://discuss.dgraph.io/t/bug-sorting-with-vars/3220

	// Test 13 (Failing)
	query = `
{
  q(func: gt(count(graduation), 1)) {
    uid
    count: count(graduation) @filter(gt(graduation, "1930"))
  }
}
`
	require.JSONEq(t,
		`{"data":{"q":[{"uid":"0x1f","count":1}]}}`,
